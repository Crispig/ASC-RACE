...
from pytorch_pretrained_bert.che_modeling import BertForMultipleChoice
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for example_index, example in enumerate(examples_iter):
        ...
        ############### #3
        # e1=example.endings[0].split(' ')
        # e2=example.endings[1].split(' ')
        # e3=example.endings[2].split(' ')
        # e4=example.endings[3].split(' ')
        # e_all=[e1,e2,e3,e4]

        # e_and=set(e_all[0]).intersection(*e_all[1:])
        # for e_item in e_all:
        #     for e in e_item:
        #         if e in e_and:
        #             e_item.remove(e)

        # for i,e in zip(range(4),e_all):
        #     example.endings[i]=' '.join(e)
        ############### #3
    ...
    ############## #0
    torch.save(features,'./f_320_orig.pt')
    ############## #0
    return features
...
def main():
    ...
    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank],
                                                          output_device=args.local_rank,find_unused_parameters=True,process_group=group)
    ############### #1

    ############### #2
    unfreeze_layers = ['layer.8','layer.9','layer.12','layer.13','layer.14','layer.15','layer.16','layer.17','layer.18','layer.19','layer.20','layer.21','layer.22','layer.23','bert.pooler','classifier']
    for name_freeze ,param_freeze in model.named_parameters():
        param_freeze.requires_grad=False
        for ele in unfreeze_layers:
            if ele in name_freeze:
                param_freeze.requires_grad=True
                break
    ############### #2
    ...
    if args.fp16:
        ...
        if args.loss_scale == 0:
            optimizer = FP16_Optimizer(optimizer, dynamic_loss_scale=True,cast_batchnorm=True,verbose=False)
        else:
    ...
    writer = SummaryWriter(os.path.join(args.output_dir, "asc032"))
    if args.do_train:
        # train_features = convert_examples_to_features(
                # train_examples, tokenizer, args.max_seq_length, True)
        
        #################### #0
        # train_features = torch.load('./f_320_orig.pt')
        ...
        train_features=[]
        for i in range(36):
            train_features=train_features+al[i]
        #################### #0
        if is_main_process():
        ...
        ############### #1
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_data,rank=torch.distributed.get_rank(),num_replicas=torch.distributed.get_world_size())
        ############### #1
    ...
    if is_main_process():
        ...
        ############# #e
        output_model_file = os.path.join(args.output_dir, "pytorch_model3.bin")
        ############# #e
